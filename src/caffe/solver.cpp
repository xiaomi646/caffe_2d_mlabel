...
void Solver<Dtype>::Init(const SolverParameter& param) {
  ...
  if (param_.has_train_net_param()) {
    //printf("param_.has_train_net_param() \n\n");
    ...
    // printf("param_.has_no_train_net_param() \n\n");
	...
	//printf("start setting  net_.reset(new Net<Dtype>(param_.train_net())) \n\n");
...
void Solver<Dtype>::Solve(const char* resume_file) {
  ...
  while (iter_++ < param_.max_iter()) {
    //LOG(INFO)<<"start forwordBackward...";
...
void Solver<Dtype>::Snapshot() {
  ...
  // Check if we need save the entire stucture or just single net(for shared weigh multiple nets)
  // modified by Tao Zeng 12/11/2014
  if (param_.snapshot_mode()==SolverParameter_SnapshotMode_SINGLE_NET)
  {
    LOG(INFO) << "serializing SnapshotMode_SINGLE_NET ";
	net_->ToProto_SingleNet(&net_param, param_.snapshot_diff());
  }else{
    LOG(INFO) << "serialing SnapshotMode_FULL_NET ";
  ...
  // Do not saving solver state for multiple net strcutures, since it is too large to save and resume
  if (param_.snapshot_mode()!=SolverParameter_SnapshotMode_SINGLE_NET){
	  SolverState state;
...
void SGDSolver<Dtype>::PreSolve() {
  ...
  LOG(INFO)<<"# of net params  = "<<net_params.size();
   // for (int i = 0; i < net_params.size(); ++i){
       // const Blob<Dtype>* net_param1 = net_params[i].get();
     // for(int j=0;j < net_params.size(); ++j)
	 // {
	   // if (i==j){continue;}
	   // const Blob<Dtype>* net_param2 = net_params[j].get();
	   // if (net_param2->data()==net_param1->data()){
	     
		  // LOG(INFO)<<"The net_param "<<i <<"use the shared data with "<<j;
...
void SGDSolver<Dtype>::ComputeUpdateValue() {
  ...
  case Caffe::GPU:
    for (int param_id = 0; param_id < net_params.size(); ++param_id) {
      ...
      Dtype local_rate = rate * net_params_lr[param_id];
      Dtype local_decay = weight_decay * net_params_weight_decay[param_id];
	  //LOG(INFO)<<"history_"<<"["<<param_id<<"]->mutable_gpu_data()";
	  ...
	  caffe_gpu_axpby(net_params[param_id]->count(), local_rate,
		  ...
		  //LOG(INFO)<<"done mutable_gpu_data()";
      if (local_decay) {
        // add weight decay
      ...
      caffe_copy(net_params[param_id]->count(),
          history_[param_id]->gpu_data(),
          net_params[param_id]->mutable_gpu_diff());
	// if out of gpu memory uncommen this line, so that history bolb will
	// copy to gpu each time when updating and be deleted from gpu to save 
	// gpu memory. By Tao Zeng 1/20/205
	 // history_[param_id]->release_data();
    }
    break;
  default:
