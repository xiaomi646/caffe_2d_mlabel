...
void Net<Dtype>::Init(const NetParameter& in_param) {
  LOG(INFO) << "Initializing net from parameters xxx : " << std::endl
  ...
  // Each input has 4 dimention (annotateby Tao Zeng)
  CHECK_EQ(param.input_size() * 4, param.input_dim_size())
      << "Incorrect input blob dimension specifications.";
  memory_used_ = 0;
  // set the input blobs
  LOG(INFO)<<"append input layer and totol input size is : " << param.input_size();
  for (int input_id = 0; input_id < param.input_size(); ++input_id) {
    const int layer_id = -1;  // inputs have fake layer ID -1
	LOG(INFO)<<"start appendTop layer "<<layer_id;
    AppendTop(param, layer_id, input_id, &available_blobs, &blob_name_to_idx);
	//LOG(INFO)<<"finish appendTop layer "<<layer_id;
  }
 // DLOG(INFO) << "Memory required for data: " << memory_used_ * sizeof(Dtype);
  // For each layer, set up their input and output
  // param.layers_size() is the total number of layers of networks
  ...
  LOG(INFO)<<"param.layer_size = "<<param.layers_size();
  for (int layer_id = 0; layer_id < param.layers_size(); ++layer_id) {
    const LayerParameter& layer_param = param.layers(layer_id);
	// GetLayer<Dtype>(layer_param)) will call from layer_factory_cpp to create different layer based on their type;
    layers_.push_back(shared_ptr<Layer<Dtype> >(GetLayer<Dtype>(layer_param)));
    layer_names_.push_back(layer_param.name());
    LOG(INFO) << "Creating Layer " << layer_param.name()<<"bottom size"<<layer_param.bottom_size();
	...
	// some ERRO occured here "basic_filebuf::underflow error reading the file in window_data_layer.cpp"
    layers_[layer_id]->SetUp(bottom_vecs_[layer_id], &top_vecs_[layer_id]);
	// LOG(INFO) << "finished Setting up " << layer_names_[layer_id];
    ...
    CHECK(param_size == num_param_blobs || param_size == 0)
        << "Incorrect param size: should be either 0 or the same as "
           "the number of the layer's parameter blobs: " << num_param_blobs<< "param size ="<<param_size<<" layer_param_.blobs_size ="<< layer_param.blobs_size()<<" blob_share_mode_size =" <<layer_param.blob_share_mode_size();
...
void Net<Dtype>::AppendTop(const NetParameter& param, const int layer_id,
  ...
  // Check if we are doing in-place computation eg. if top==bottom do in-place 
  if (layer_param && layer_param->bottom_size() > top_id &&
    ...
    if (layer_id == -1) {
	  ...
	  //LOG(INFO)<<"trying to get blob data";
      top_vecs_[layer_id].push_back(blob_pointer.get());
	  //LOG(INFO)<<"blob data abtained";
    }
    memory_used_ += blob_pointer->count();
	//LOG(INFO)<<"memory_used_ += blob_pointer->count()";
  }
  available_blobs->insert(blob_name);
  //LOG(INFO)<<"available_blobs->insert(blob_name)()";
}
...
void Net<Dtype>::AppendParam(const NetParameter& param, const int layer_id,
  ...
  if (!param_size || !param_name.size() || (param_name.size() &&
	...
	// release occupied gpu data;
	layers_[layer_id]->blobs()[param_id]->release_data();
	
    layers_[layer_id]->blobs()[param_id]->ShareData(
        *layers_[owner_layer_id]->blobs()[owner_param_id]);
    
	
	Blob<Dtype>* cur_blob = layers_[layer_id]->blobs()[param_id].get();
    Blob<Dtype>* own_blob =
        layers_[owner_layer_id]->blobs()[owner_param_id].get();
	
	
	CHECK_EQ(cur_blob->data(), own_blob->data());
  }
}
...
const vector<Blob<Dtype>*>& Net<Dtype>::ForwardPrefilled(Dtype* loss) {
  ...
  for (int i = 0; i < layers_.size(); ++i) {
	...
	//LOG(INFO)<<"start forwarding in " << layer_names_[i];
...
void Net<Dtype>::CopyTrainedLayersFrom(const NetParameter& param) {
  int num_source_layers = param.layers_size();
   LOG(INFO)<<"Number of soure layers are "<<num_source_layers ;
  for (int i = 0; i < num_source_layers; ++i) {
    ...
    CHECK_EQ(target_blobs.size(), source_layer.blobs_size())
        << "Incompatible number of blobs for layer " << source_layer_name;
   // LOG(INFO)<<"Number of target_blobs are "<<target_blobs.size();
    for (int j = 0; j < target_blobs.size(); ++j) {
...
template <typename Dtype>
void Net<Dtype>::CopyTrainedLayersWithMoreSourceChannelFrom(const NetParameter& param){
	int num_source_layers = param.layers_size();
	//int layer_names_.size() =layer_names_.size();
   LOG(INFO)<<"Number of soure layers are "<<num_source_layers ;
   map<string, int> shared_params_idx;
   int idexCount =0;
   
   // ONLY For testing how many blob's data are shared 
  for(int t=0; t<layer_names_.size();++t){
            int target_layer_id=t;
			vector<shared_ptr<Blob<Dtype> > >& target_blobs =
					layers_[t]->blobs();
			for (int j=0;j<layer_names_.size();++j){
			    vector<shared_ptr<Blob<Dtype> > >& target_blobs_2 =
					layers_[j]->blobs();
				if(t==j){continue;}
				if (layer_names_[t]=="conv1_1p" && layer_names_[j]=="conv1_1"){
				Blob<Dtype>* blob1 = target_blobs[0].get();
				Blob<Dtype>* blob2 = target_blobs_2[0].get();
				//const shared_ptr<SyncedMemory>& d1=blob1->data();
				//const shared_ptr<SyncedMemory>& d2 =blob2->data();
					if(blob2->data() ==blob1->data())
					{LOG(INFO)<<"<conv1_1p> point to the same data pointed by <conv1_1> ";
					
					}else{
					  LOG(INFO)<<"<conv1_1p> point to the dif data pointed by <conv1_1>";
					}
				}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  for (int i = 0; i < num_source_layers; ++i) {
	...
	for(int t=0; t<layer_names_.size();++t){                    		// remove this to original
		if (layer_names_[t].size()>=source_layer_name.size())           //for example: in case that target layer names is "fc_xxx" and source name is "fc";
			{
		     int source_layer_name_strlen=source_layer_name.size();			 //
			 //LOG(INFO)<<"surce_layer_name "<<source_layer_name <<"="<<source_layer_name.size()<<" target = " <<layer_names_[t].substr(0,strlen-1);
			  target_layer_id=t;
			vector<shared_ptr<Blob<Dtype> > >& target_blobs =
					layers_[target_layer_id]->blobs();
			//const int num_param_blobs = layers_[target_layer_id]->blobs().size();
			
			
			if (layer_names_[t].substr(0,source_layer_name_strlen)==source_layer_name && target_blobs.size()== source_layer.blobs_size()){ //
			   ...
			   const int param_size = layer_param.param_size();
				...
				for (int j = 0; j < target_blobs.size(); ++j) {
				  // This part avoids copying shared weight from the source(pre-trained)/ tzeng 1/13/2014
				  if (param_size) {
					  string param_name=layer_param.param(j);
					  LOG(INFO)<<"temp params array size =  " <<shared_params_idx.size()  <<" and found index for "<<param_name ;
					  
					  if (shared_params_idx.empty()){
					      shared_params_idx[param_name]=idexCount;
						  idexCount++;
					  }else if (shared_params_idx.find(param_name) != shared_params_idx.end()){
					     LOG(INFO)<< "blob's weight param "<<param_name <<" is a shared weight";
						 //LOG(INFO)<< "blob's weight param "<<param_name <<" is a shared weight, skipping copying from source";
						 //continue;
					  }else{
					    shared_params_idx[param_name]=idexCount;
						idexCount++;
				  ...
				  LOG(INFO)<<"target blobs N x C x H x W="<<target_blobs[j]->num()<<"X"<<target_blobs[j]->channels()
				  <<"X"<<target_blobs[j]->height()<<"X"<<target_blobs[j]->width();
				  
				  LOG(INFO)<<"source blobs N x C x H x W="<<source_layer.blobs(j).num()<<"X"<<source_layer.blobs(j).channels()
				  <<"X"<<source_layer.blobs(j).height()<<"X"<<source_layer.blobs(j).width();
				  
				  if((target_blobs[j]->num()!= source_layer.blobs(j).num()) || (target_blobs[j]->height()!= source_layer.blobs(j).height()) || (target_blobs[j]->width()!= source_layer.blobs(j).width())){
				    LOG(INFO) <<"bolb size do not match, ignoring the weight copying ... ";
					continue;
				  ...
				  CHECK_EQ(target_blobs[j]->height(), source_layer.blobs(j).height());
				  CHECK_EQ(target_blobs[j]->width(), source_layer.blobs(j).width());
				  LOG(INFO)<<"current layer ="<<i <<" and current blobs number = "<< j;
				  //if ((i==0) && (target_blobs[j]->channels()>source_layer.blobs(j).channels()))
				   LOG(INFO) << "copied source layer " << source_layer_name << " To target layer "<< layer_names_[target_layer_id];
				  if (target_blobs[j]->channels()>source_layer.blobs(j).channels())
				  {
					// this intend to deal with the case that target has mutilple channel while a source(pretrained) has only 3chanels(1 rgb images)
					// in which, it will duplicate the RGB chanels'weight to other images
					  target_blobs[j]->FromProtoWithExtraCopy(source_layer.blobs(j));
				  }else{
					  CHECK_EQ(target_blobs[j]->channels(), source_layer.blobs(j).channels()); 
					  target_blobs[j]->FromProto(source_layer.blobs(j));
				  }
...
template <typename Dtype>
void Net<Dtype>::CopyTrainedLayersWithMoreSourceChannelFrom(const string trained_filename) {
  NetParameter param;
  ReadNetParamsFromBinaryFileOrDie(trained_filename, &param);
  CopyTrainedLayersWithMoreSourceChannelFrom(param);
}
...
// This code is implemented for serialize just a single net out of other 
// shared weight nets. so it can be save and reloaded as model afterword without
// encountering space or memory problem.
// Implemented by Tao Zeng12/11/2014
template <typename Dtype>
void Net<Dtype>::ToProto_SingleNet(NetParameter* param, bool write_diff) {
  param->Clear();
  param->set_name(name_);
  // Add bottom and top
  for (int i = 0; i < net_input_blob_indices_.size(); ++i) {
  ...
  LOG(INFO) << "Serializing " << layers_.size() << " layers" << "  net_input_blob_indices_.size =" << net_input_blob_indices_.size();
  for (int i = 0; i < layers_.size(); ++i) {
    //LOG(INFO) << "serializing Layer "<< i << "name =" << layer_names_[i];
    LayerParameter* layer_param = param->add_layers();
	string l_name =layer_names_[i];
	if ( (l_name[l_name.size()-1]=='p') ||(l_name =="concat_layer") || (l_name =="loss")||(l_name=="fc8_gene_ontology"))
	{
	   // skip saving duplicated layers of other nets (name with 'p' in the last character)
	     LOG(INFO) <<"skip serializing layer " << layer_names_[i];
		continue;
	}
    for (int j = 0; j < bottom_id_vecs_[i].size(); ++j) {
      layer_param->add_bottom(blob_names_[bottom_id_vecs_[i][j]]);
