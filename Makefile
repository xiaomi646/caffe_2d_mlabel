...
COMMON_FLAGS += $(foreach includedir,$(INCLUDE_DIRS),-I$(includedir) -I/usr/local/cuda/include)
CXXFLAGS += -pthread -fPIC $(COMMON_FLAGS) $(WARNINGS)
NVCCFLAGS += -ccbin=$(CXX) -Xcompiler -fPIC $(COMMON_FLAGS)
# mex may invoke an older gcc that is too liberal with -Wuninitalized
MATLAB_CXXFLAGS := $(CXXFLAGS) -Wno-uninitialized
LINKFLAGS += -fPIC $(COMMON_FLAGS) $(WARNINGS)
LDFLAGS += $(foreach librarydir,$(LIBRARY_DIRS),-L$(librarydir)) -L/usr/local/cuda/lib \
		$(foreach library,$(LIBRARIES),-l$(library)) 
# -ldl -lrt -lculibos -lcurand_static -lcudart_static -lcublas_static)
