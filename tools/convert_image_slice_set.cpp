...
int main(int argc, char** argv) {
  ...
  if (argc < 4 || argc > 9) {
    printf("Convert a set of images to the leveldb format used\n"
        "as input for Caffe.\n"
        "Usage:\n"
        "    convert_imageset [-g] [-r ROOTFOLDER/] LISTFILE DB_NAME"
  ...
  bool is_root_dir_need= (string("-r") == string(argv[1+arg_offset]));
       arg_offset = (is_root_dir_need ? arg_offset+1 : arg_offset-1);
	   
  //std::vector<std::pair<std::vector<string>, std::vector<int> > >
  std::vector<std::string> imageSetIDs;
  std::vector<std::pair<std::vector<string>, std::vector<int> > > lines;// image name and corespoding labels in the line
  ReadImagesList(argv[arg_offset+2], &lines,imageSetIDs);
  ...
  if (argc >= (arg_offset+6)) {
	...
	LOG(INFO) << "(db_type) arg_offset +5 is " <<db_backend;
  ...
  LOG(INFO) << "resize height = " <<resize_height <<"resize width =" <<resize_width;
  // Open new db
  // lmdb
  MDB_env *mdb_env;
  MDB_dbi mdb_dbi;
  ...
  for (int line_id = 0; line_id < lines.size(); ++line_id) {
	...
	if (!ReadImageToDatum(lines[line_id].first,
        lines[line_id].second, resize_height, resize_width, is_color, &datum)) {
      continue;
    }
    ...
    // sequential
	string secID =imageSetIDs[line_id];
	
    //snprintf(key_cstr, kMaxKeyLength, "%08d_%s", line_id,
    //    lines[line_id].first.c_str());
	 string keyString =secID;
	 //snprintf(key_cstr, kMaxKeyLength, "%08d_%s", line_id,
     //  secID.c_str());
    ...
    LOG(INFO)<<"writing serial ID = "<<keyString<<" # ="<<line_id;
    // Put in db
    if (db_backend == "leveldb") {  // leveldb
