...
class Net {
 public:
  ...
  // this function will just copy rest of weighs correspoding to one images RBG channel to all other channels
   void CopyTrainedLayersWithMoreSourceChannelFrom(const string trained_filename);
   void CopyTrainedLayersWithMoreSourceChannelFrom(const NetParameter& param);
  //========================================================================================================================= 
  // Writes the net to a proto.
  void ToProto(NetParameter* param, bool write_diff = false);
  //Wrties the just one shared weight net to proto out of all other nets with the same weight;
  // By Tao Zeng 12/11/2014
  void ToProto_SingleNet(NetParameter* param, bool write_diff = false);
